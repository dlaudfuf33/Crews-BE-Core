...
-- CoreTransaction 테이블 생성 (순환 참조 제거)
...
(
    ...
    counterparty_account_num VARCHAR(20),
    counterparty_bank_code   VARCHAR(20),
    description              VARCHAR(50)             DEFAULT '',
    created_at               DATETIME       NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at               DATETIME       NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    core_transaction_id      BIGINT,                               -- transaction을 참조하는 외래 키
    FOREIGN KEY (core_transaction_id) REFERENCES transaction (id), -- 참조는 core_history에서만 수행
